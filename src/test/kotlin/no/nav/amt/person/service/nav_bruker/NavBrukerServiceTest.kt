...
class NavBrukerServiceTest {
	...
	private val service = NavBrukerService(
	...
	fun `hentEllerOpprettNavBruker - bruker finnes ikke - oppretter og returnerer ny bruker`() {
		...
		every { poaoTilgangClient.erSkjermetPerson(person.personident) } returns ApiResult(
			result = erSkjermet,
			throwable = null
		)
	...
	fun `syncKontaktinfoBulk - telefon er registrert i krr - oppdaterer bruker med telefon fra krr`() {
		...
		every { krrProxyClient.hentKontaktinformasjon(setOf(bruker.person.personident)) } returns Result.success(
	...
	fun `syncKontaktinfoBulk - krr feiler - oppdaterer ikke`() {
		...
		every { brukerRepository.finnBrukerId(bruker.person.personident) } returns bruker.id
		every { brukerRepository.get(bruker.person.personident) } returns bruker
		every { krrProxyClient.hentKontaktinformasjon(setOf(bruker.person.personident)) } returns Result.failure(
			RuntimeException()
		)
	...
	fun `oppdaterKontaktinformasjon - krr feiler - oppdaterer ikke`() {
		...
		every { brukerRepository.get(bruker.person.personident) } returns bruker
		every { krrProxyClient.hentKontaktinformasjon(bruker.person.personident) } returns Result.failure(
			RuntimeException()
		)
		...
		verify(exactly = 1) {
			brukerRepository.upsert(match {
				it.oppfolgingsperioder.size == 2 &&
					it.oppfolgingsperioder.find { oppfolgingsPeriode -> oppfolgingsPeriode.id == bruker.oppfolgingsperioder.first().id } == bruker.oppfolgingsperioder.first() &&
		...
		verify(exactly = 1) {
			brukerRepository.upsert(match {
				it.oppfolgingsperioder.size == 1 &&
					it.oppfolgingsperioder.find { oppfolgingsPeriode -> oppfolgingsPeriode.id == bruker.oppfolgingsperioder.first().id } == oppfolgingsperiode
			})
		}
	}
